...
def get_patch(ms_image, lms_image, pan_image, bms_image, patch_size, scale, ix=-1, iy=-1):
    ...
    # if ix == -1:
    #     ix = random.randrange(0, iw - ip + 1)   #取一个随机数
    # if iy == -1: 
...
class Data(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #ms(w,h)
        ms_image = ms_image.crop((0, 0, ms_image.size[0] // self.upscale_factor * self.upscale_factor, ms_image.size[1] // self.upscale_factor * self.upscale_factor))#图像裁剪，使得能被尺度整除
        ...
        #应该是降采样后的pan,size(w,h)
        bms_image = rescale_img(lms_image, self.upscale_factor)#size(w,h)
 
        #ms_image, lms_image, pan_image, bms_image, _ = get_patch(ms_image, lms_image, pan_image, bms_image, self.patch_size, scale=4)
