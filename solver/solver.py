...
from utils.utils import maek_optimizer, make_loss, calculate_psnr, calculate_ssim, save_config, save_net_config, DIRCNN_data_preprocess
...
class Solver(BaseSolver):
    def __init__(self, cfg):
        ...
        self.optimizer = maek_optimizer(
            self.cfg['schedule']['optimizer'], cfg, self.model.parameters())
        self.loss = make_loss(self.cfg['schedule']['loss'])

        self.log_name = self.cfg['algorithm'] + '_' + \
            str(self.cfg['data']['upsacle']) + '_' + str(self.timestamp)
        # save log
        ...
        save_config(self.log_name, 'Train dataset has {} images and {} batches.'.format(
            len(self.train_dataset), len(self.train_loader)))
        #save_config(self.log_name, 'Val dataset has {} images and {} batches.'.format(len(self.val_dataset), len(self.val_loader)))
        save_config(self.log_name, 'Model parameters: ' +
                    str(sum(param.numel() for param in self.model.parameters())))
    ...
    def train(self):
        with tqdm(total=len(self.train_loader), miniters=1,
            ...
            for iteration, batch in enumerate(self.train_loader, 1):
                ms_image, lms_image, pan_image, bms_image, file = Variable(batch[0]), Variable(
                ...
                #print('fusion image{},ms image{}'.format(y,ms_image))
            ...
            self.writer.add_image('image1_ms1', ms_image[0], self.epoch)
            self.writer.add_image('image2_fusion', y[0], self.epoch)
            self.writer.add_image('image3_pan', pan_image[0], self.epoch)

            save_config(self.log_name, 'Initial Training Epoch {}: Loss={:.4f}'.format(
                self.epoch, self.records['Loss'][-1]))
            self.writer.add_scalar(
                'Loss_epoch', self.records['Loss'][-1], self.epoch)
    ...
    def eval(self):
        with tqdm(total=len(self.val_loader), miniters=1,
            ...
            for iteration, batch in enumerate(self.val_loader, 1):
                ...
                for c in range(y.shape[0]):
                    if not self.cfg['data']['normalize']:
                        predict_y = (
                            y[c, ...].cpu().numpy().transpose((1, 2, 0))) * 255
                        ground_truth = (ms_image[c, ...].cpu(
                        ).numpy().transpose((1, 2, 0))) * 255
                    else:
                        predict_y = (y[c, ...].cpu().numpy(
                        ).transpose((1, 2, 0)) + 1) * 127.5
                        ground_truth = (ms_image[c, ...].cpu(
                        ).numpy().transpose((1, 2, 0)) + 1) * 127.5
                ...
                t1.set_postfix_str('Batch loss: {:.4f}, PSNR: {:.4f}, SSIM: {:.4f}'.format(
            ...
            save_config(self.log_name, 'Val Epoch {}: PSNR={:.4f}, SSIM={:.4f}'.format(self.epoch, self.records['PSNR'][-1],
                                                                                       self.records['SSIM'][-1]))
            self.writer.add_scalar(
                'PSNR_epoch', self.records['PSNR'][-1], self.epoch)
            self.writer.add_scalar(
                'SSIM_epoch', self.records['SSIM'][-1], self.epoch)
    ...
    def check_pretrained(self):
        checkpoint = os.path.join(
            self.cfg['pretrain']['pre_folder'], self.cfg['pretrain']['pre_sr'])
        if os.path.exists(checkpoint):
            self.model.load_state_dict(torch.load(
                checkpoint, map_location=lambda storage, loc: storage)['net'])
            self.epoch = torch.load(
                checkpoint, map_location=lambda storage, loc: storage)['epoch']
    ...
    def save_checkpoint(self):
        ...
        torch.save(self.ckp, os.path.join(
            self.cfg['checkpoint'] + '/' + str(self.log_name), 'latest.pth'))
